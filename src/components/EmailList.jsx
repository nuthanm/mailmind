...
export default function EmailList() {
  ...
  const requestedTopCount = topMode
    ? parseInt(String(minSizeKb).replace("top_", ""), 10) || 0
    : 0;

  const senderOptions = useMemo(() => {
    const unique = [
      ...new Set(
        filteredEmails.map((e) => e.from_email || e.from_name || "Unknown"),
      ),
    ];
  ...
  const viewEmails = useMemo(() => {
    ...
    const list = source.filter((e) => estimateSizeKb(e) >= threshold).slice();

    list.sort((a, b) => {
      if (topMode) return estimateSizeKb(b) - estimateSizeKb(a);
      if (sizeSort === "size_desc")
        return estimateSizeKb(b) - estimateSizeKb(a);
      if (sizeSort === "size_asc") return estimateSizeKb(a) - estimateSizeKb(b);
      return (
        new Date(b.received_at || b.created_at || 0) -
        new Date(a.received_at || a.created_at || 0)
      );
    });
  ...
  useEffect(() => {
    const needsMore =
      topMode &&
      requestedTopCount > 0 &&
      scopedEmails.length < requestedTopCount;
    ...
    const run = async () => {
      ...
      try {
        if (activeAccountId !== "all") {
          ...
          await withTimeout(
            fetchEmailsSSE(activeAccountId, {
              query: "in:inbox",
              maxEmails: target,
            }),
            120000,
          ...
          const perAccount = Math.min(
            1000,
            Math.max(
              Math.ceil(requestedTopCount / Math.max(gmailAccounts.length, 1)) *
                4,
              250,
            ),
          );
          for (const acc of gmailAccounts) {
            await withTimeout(
              fetchEmailsSSE(acc.id, {
                query: "in:inbox",
                maxEmails: perAccount,
              }),
              120000,
              `Top view fetch for ${acc.email}`,
            );
          }
        }
      } catch (e) {
  ...
  const selectableBySender = useMemo(() => {
    if (senderGroups.size > 0) {
      return viewEmails.filter((e) =>
        senderGroups.has(e.from_email || e.from_name || "Unknown"),
      );
    }
    return viewEmails;
  ...
  const deleteAllFromSender = () => {
    ...
    const ids = selectableBySender.map((e) => e.id);
    const totalKb = selectableBySender.reduce(
      (sum, e) => sum + estimateSizeKb(e),
      0,
    );
    const senderCount = senderGroups.size;
    const senderLabel =
      senderCount === 1 ? [...senderGroups][0] : `${senderCount} senders`;
    setSelectedIds(new Set(ids));
    setDeleteModal({
      ids,
  ...
  const deleteSelected = () => {
    ...
    setDeleteModal({
      ...
      message:
        "All selected emails will be removed from MailMind. Gmail-linked emails will be moved to Gmail Trash.",
  ...
  const deleteFiltered = () => {
    ...
    setDeleteModal({
      ...
      message:
        "All emails in the current filter will be removed from MailMind. Gmail-linked emails will be moved to Gmail Trash.",
    ...
    return (
      <div className="email-list">
        {[...Array(4)].map((_, i) => (
          <div
            key={i}
            className="email-card skeleton-card"
      ...
      <div className="email-list">
        <div className="email-controls">
          <div className="email-controls-left">
            <div className="sender-multi" ref={senderMenuRef}>
              <button
                className="sender-multi-btn"
                onClick={() => !uiLocked && setSenderMenuOpen((v) => !v)}
                disabled={uiLocked}
              >
                {senderGroups.size === 0
                  ? "Sender group: All senders"
                  : `Senders selected: ${senderGroups.size}`}
              </button>
              {senderMenuOpen && (
                <div className="sender-multi-menu">
                  <label className="sender-opt">
                    <input
                      type="checkbox"
                      checked={senderGroups.size === 0}
                      onChange={clearSenderGroups}
                      disabled={uiLocked}
                    />
                  ...
                  {senderOptions.map((s) => (
                    <label key={s} className="sender-opt">
                      <input
                        type="checkbox"
                        checked={senderGroups.has(s)}
                        onChange={() => toggleSenderGroup(s)}
                        disabled={uiLocked}
                      />
                      <span>{s}</span>
                    </label>
        ...
        <div className="empty-state">
          ...
          <div className="empty-sub">
            No emails match the current filters. Try lowering the minimum size
            filter or load emails first.
          </div>
        </div>
      </div>
    );
    ...
    <div className={`email-list${deleting ? " deleting" : ""}`}>
      ...
      <div className="email-controls">
        <div className="email-controls-left">
          ...
          <button
            className="btn-ghost"
            onClick={selectSenderGroup}
            disabled={uiLocked || selectableBySender.length === 0}
          >
            Select Sender
          </button>
          {senderGroups.size > 0 && (
          ...
          <button
            ...
            disabled={uiLocked || viewEmails.length === 0}
          >
            Select Visible
          </button>
          <button
        ...
        <div className="email-controls-right">
          <span className="email-selected-count">
      ...
      <div className="email-status-strip">
        <span className="email-filter-size">
          Filtered size: {formatKb(filteredSizeKb)}
        </span>
        {senderGroups.size > 0 && (
          <div className="sender-chip-row">
            {[...senderGroups].map((s) => (
              <span key={s} className="sender-chip">
                {s}
                <button
                  className="sender-chip-x"
                  onClick={() => removeSenderGroup(s)}
                  disabled={uiLocked}
                >
                  x
                </button>
        ...
        {topFilterInfo.active && (
          <span className="email-filter-note">
            Biggest view: {viewEmails.length}/{topFilterInfo.requested}
          </span>
        )}
      </div>
      ...
      <div className={`email-list-content${uiLocked ? " is-deleting" : ""}`}>
        {selectedIds.size > 0 && (
          <div className="email-selection-bar">
            <span>
              ✓ <strong>{selectedIds.size}</strong> email
              {selectedIds.size !== 1 ? "s" : ""} selected
            </span>
            <span className="sel-bar-sep">·</span>
            <span>
              Estimated space freed: <strong>{formatKb(selectedSizeKb)}</strong>
            </span>
            <span className="sel-bar-sep">·</span>
            <button
        ...
        {selectedIds.size === 0 && (
          <div className="email-controls-note">
            Tip: Pick a sender group, click Select Sender, uncheck any to keep,
          ...
          <div className="confirm-card">
            ...
            {deleteModal.spaceKb > 0 && (
              <div className="confirm-space-badge">
                ...
                <strong>{formatKb(deleteModal.spaceKb)}</strong>
              </div>
            )}
              ...
              <div className="confirm-email-list">
                {deleteModal.emails.slice(0, 8).map((e) => (
                  <div key={e.id} className="confirm-email-row">
                    <span className="confirm-email-sender">
                      {e.from_name || e.from_email || "Unknown"}
                    </span>
                    <span className="confirm-email-subject">
                      {e.subject || "(no subject)"}
                    </span>
                    <span className="confirm-email-size">
                      {formatKb(estimateSizeKb(e))}
                    </span>
                  </div>
                ))}
                {deleteModal.emails.length > 8 && (
                  <div className="confirm-email-more">
                    +{deleteModal.emails.length - 8} more email
                    {deleteModal.emails.length - 8 !== 1 ? "s" : ""}
                  </div>
                )}
              </div>
            )}
...
function EmailCard({
  ...
  if (email.fields) {
    tags.push(
      <span key="ex" className="tag tag-success">
        ✓ Extracted
      </span>,
    );
    if (email.fields.priority === "High")
      tags.push(
        <span key="hi" className="tag tag-danger">
          High priority
        </span>,
      );
    else if (email.fields.action_required === "Yes")
      tags.push(
        <span key="act" className="tag tag-warning">
          Action needed
        </span>,
      );
    if (email.draft && !email.draft_sent)
      tags.push(
        <span key="dr" className="tag tag-info">
          Draft ready
        </span>,
      );
    if (email.draft_sent)
      tags.push(
        <span key="sent" className="tag tag-muted">
          Sent ✓
        </span>,
      );
  } else {
    tags.push(
      <span key="un" className="tag tag-muted">
        Pending
      </span>,
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="em-avatar" style={{ background: bg, color: fg }}>
        {initials}
      </div>
      <div className="em-body">
        ...
        <div className="em-preview">
          {email.body.substring(0, 90).replace(/\n/g, " ")}…
        </div>
      </div>
