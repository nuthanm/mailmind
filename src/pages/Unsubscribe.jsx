...
export default function Unsubscribe() {
  ...
  const scan = async () => {
    ...
    try {
      ...
      await load();
      showToast(
        `✓ Scan complete — found ${data.detected} newsletter sender${data.detected !== 1 ? "s" : ""}`,
      );
    } catch (e) {
      showToast(e.message, "error");
    } finally {
  ...
  const queueSelected = async () => {
    ...
    await load();
    showToast(
      `${selected.size} sender${selected.size !== 1 ? "s" : ""} queued`,
    );
  };
  ...
  const removeSelected = async () => {
    ...
    await load();
    showToast(
      `${ids.length} sender${ids.length !== 1 ? "s" : ""} removed from list`,
    );
  };
  ...
  const removeAllInFilter = async () => {
    ...
    try {
      ...
      await load();
      showToast(
        `${filtered.length} sender${filtered.length !== 1 ? "s" : ""} removed from current filter`,
      );
    } catch (e) {
      showToast(e.message, "error");
    } finally {
  ...
  const fireOne = async (id) => {
    ...
    try {
      ...
      if (data.status === "done") {
        ...
        showToast(
          "⚠ Please check your email to confirm unsubscribe",
          "warning",
        );
      } else {
  ...
  const fireAll = async () => {
    ...
    try {
      ...
      await load();
      showToast(
        `✓ ${data.succeeded} unsubscribed, ${data.failed} failed`,
        "success",
      );
    } catch (e) {
      showToast(e.message, "error");
  ...
  const selectAll = () => {
    const ids = filtered
      .filter(
        (s) =>
          ["detected", "queued", "blocked"].includes(s.status) &&
          s.unsub_method !== "none",
      )
      .map((s) => s.id);
    setSelected((prev) =>
  ...
  const getFilterCount = (id) => {
    if (!stats) return 0;
    if (id === "blocked")
      return senders.filter((s) => s.status === "blocked").length;
    return stats[id] || 0;
  };
  ...
  const filtered = senders
    ...
    .sort((a, b) => {
      ...
      if (sortBy === "size")
        return (b.total_size_kb || 0) - (a.total_size_kb || 0);
      if (sortBy === "count")
  ...
  return (
    <div className="unsub-view">
      ...
      <div className="unsub-info-banner">
        <span className="unsub-info-icon">ℹ</span>
        <span>
          <strong>How it works:</strong> Clicking "Unsub" sends an unsubscribe
          request to the sender. It may take <strong>5–10 business days</strong>{" "}
          for emails to stop arriving. Some senders require you to{" "}
          <strong>confirm via email</strong> — check your inbox if emails
          continue.
        </span>
      </div>
      {/* HEADER */}
      <div className="unsub-header">
        <div className="unsub-header-left">
          ...
          <div className="unsub-desc">
            Detect newsletters and promotional emails, then unsubscribe in one
            click. Estimated storage saved is shown per sender.
          </div>
        </div>
        <div className="unsub-header-actions">
          ...
          <button className="btn-ghost" onClick={scan} disabled={scanning}>
            {scanning ? (
              <>
                <div className="spin-inline"></div> Scanning…
              </>
            ) : (
              "🔍 Scan Inbox"
            )}
        ...
        >
          ...
          {filtered.length > 0 && (
            ...
            >
              {removingAll
                ? "Removing…"
                : `Remove All In Filter (${filtered.length})`}
            </button>
          )}
          {filtered.some(
            (s) => s.status === "detected" && s.unsub_method !== "none",
          ) && (
            <button className="btn-ghost" onClick={queueAll}>
              Queue All →
            </button>
          )}
        </div>
            ...
            {filtered.some(
              ...
              <div className="unsub-select-row">
                <label className="checkbox-label" onClick={selectAll}>
                  <div
                    className={`checkbox ${selected.size === filtered.filter((s) => ["detected", "queued", "blocked"].includes(s.status) && s.unsub_method !== "none").length ? "checked" : ""}`}
                  >
                    {selected.size > 0 && "✓"}
                  </div>
                  <span>
...
function SenderRow({
  ...
  const canQueue =
    (sender.status === "detected" || sender.status === "blocked") &&
  ...
  return (
    ...
    >
      <div className="sender-row-main" onClick={() => setExpanded(!expanded)}>
        ...
        {sender.status === "done" && (
          <div className="checkbox checked done-check">✓</div>
        )}
        {["failed", "blocked", "processing"].includes(sender.status) && (
        ...
        <div className="sender-info">
          <div className="sender-name-row">
            ...
            <span className="sender-type-badge">
              {TYPE_ICON[sender.sender_type]} {sender.sender_type}
            </span>
          </div>
        ...
        <div className="sender-stats">
          ...
          <div className="sender-stat">
            <span className="sender-stat-val">
              {formatKb(sender.total_size_kb)}
            </span>
        ...
        <div className="sender-actions" onClick={(e) => e.stopPropagation()}>
          ...
          {sender.status === "done" && (
            <span
              className="done-note"
              title="Unsubscribe request sent. It can take 5–10 business days for emails to stop. Check your inbox for any confirmation emails from the sender."
            >
              Done ✓ · may take a few days
            </span>
          )}
          {sender.status === "failed" && (
            <button className="act-btn retry" onClick={onFire} title="Retry">
              ↺ Retry
            </button>
          )}
          <button
            className="act-btn ignore"
            onClick={onBlock}
          ...
          <div className="detail-grid">
            ...
            <div>
              ...
              <div className="detail-val">
                {new Date(sender.last_seen_at).toLocaleDateString()}
              </div>
            </div>
            <div>
            ...
            {sender.error_msg && (
              <div style={{ gridColumn: "1/-1" }}>
                <div className="detail-label" style={{ color: "var(--red)" }}>
                  Last error
                </div>
...
function StatBadge({ icon, label, value, color }) {
  return (
    <div className="unsub-stat-badge">
      <div className="unsub-stat-val" style={{ color }}>
        {value}
      </div>
      <div className="unsub-stat-lbl">
        {icon} {label}
      </div>
    </div>
  );
